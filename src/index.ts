...
program
	.name('gemit')
	.description(
		style('Suggest commit messages and branch names with AI', ui.cyan),
	)
...
program
	...
	.option('--check', 'Run lint/test (if present) before creating commit')
	.action((options: { all?: boolean; check?: boolean }) =>
		addAndCommit(options),
	)
...
program
	.command('log')
	.description('Summarize what was done in the current branch')
	.action(summarizeBranchLog)
...
program
	...
	.action((name: string | undefined, options: { commits?: string }) =>
		generateChangelog(name, options),
	)
...
program
	...
	.option(
		'--set <key=value>',
		'Set a config value (e.g. --set LLM_MODEL=gpt-4o)',
	)
...
program
	.command('update')
	.description(
		'Force a check for updates and install if a new version is available',
	)
	.action(async () => {
...
program
	...
	.option(
		'--init',
		'Export built-in prompts to the global config directory and open it',
	)
	.option(
		'--edit <name>',
		'Open a specific prompt file in your default editor (commit|branch|pr|changelog|log)',
	)
	.option(
		'--show <name>',
		'Print the built-in template for a specific prompt (commit|branch|pr|changelog|log)',
	)
