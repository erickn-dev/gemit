...
/*
 * Render a tree-style list of key/value rows.
 */
export function printKeyValues(rows: KeyValueRow[]): void {
	...
	for (let i = 0; i < rows.length; i++) {
		...
		const connector = style(
			isLast ? symbols.treeLast : symbols.treeItem,
			ui.gray,
		)
...
/*
 * Render a tree-style list with a title and zero or more items.
 */
export function printList(title: string, items: string[]): void {
	...
	if (items.length === 0) {
		console.log(
			`  ${style(symbols.treeLast, ui.gray)} ${style('(none)', ui.gray, ui.italic)}`,
		)
		return
	}
...
/*
 * Render a framed error block and exit the process.
 */
