...
function resolveBaseRef(): string {
	...
	failAndExit(
		'Could not infer base branch. Set a remote HEAD or create main/master.',
	)
}
...
export function getBranchContext(): BranchContext {
	...
	const patchRaw = execFileSync('git', ['diff', `${mergeBase}..HEAD`], {
		stdio: 'pipe',
	})
		.toString()
		.trim()
	...
	const patch = patchTruncated
		? patchRaw.slice(0, MAX_BRANCH_PATCH_CHARS)
		: patchRaw
...
export function getCommitRangeDiffStat(limit: number): string {
	try {
		return execFileSync('git', ['diff', `HEAD~${limit}..HEAD`, '--stat'], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
...
export function getCommitsSinceLastRelease(fallbackLimit = 50): ReleaseRange {
	...
	try {
		const raw = execFileSync(
			...
			[
				'log',
				'--reverse',
				'--pretty=format:%H%x1f%s%x1f%b%x1f%ad%x1f%an%x1e',
				'--date=short',
				rangeArg,
			],
			{ stdio: 'pipe' },
		...
		commits = raw
			...
			.filter(Boolean)
			.map((entry) => {
				const [hash = '', subject = '', body = '', date = '', author = ''] =
					entry.split('\x1f')
				return {
					hash: hash.trim(),
					subject: subject.trim(),
	...
	let diffStat = ''
	try {
		diffStat = execFileSync('git', ['diff', '--stat', rangeArg], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
