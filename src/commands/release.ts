...
/*
 * JSON files where version is a top-level key, such as package.json or composer.json.
 */
...
/*
 * TOML files such as Cargo.toml or pyproject.toml.
 */
...
/*
 * setup.cfg version field without quotes.
 */
...
/*
 * pubspec.yaml version field for Dart and Flutter projects.
 */
...
/*
 * Return every project file that actually exists on disk.
 */
function detectProjectFiles(): ProjectFile[] {
	const candidates: ProjectFile[] = [
		jsonProjectFile('package.json'),
		jsonProjectFile('composer.json'),
		tomlProjectFile('Cargo.toml', 'Cargo.toml'),
		tomlProjectFile('pyproject.toml', 'pyproject.toml'),
		setupCfgProjectFile(),
		pomXmlProjectFile(),
		pubspecYamlProjectFile(),
	]
...
export async function createRelease(version?: string): Promise<void> {
	...
	if (!version || !/^\d+\.\d+\.\d+(-[\w.]+)?$/.test(version)) {
		failAndExit(
			`Invalid version format: "${version}". Use semver like 1.2.3 or 1.2.3-beta.1`,
		)
	}
	...
	const generateChangelog = await askConfirmation(
		`Generate AI changelog for ${tag}? (y/N): `,
		{ defaultYes: false },
	)
	...
	if (generateChangelog) {
		...
		const rangeLabel = fromTag
			? `${fromTag}..HEAD`
			: 'início do repositório..HEAD'
		...
		const changelogPrompt = interpolate(changelogTemplate, {
			...
			commit_history:
				commitHistory || '(nenhum commit novo desde a última release)',
			diff_stat: diffStat || '(sem alterações de arquivos)',
		})

		const result = await withProgress('AI is generating changelog...', () =>
			llm.invoke(changelogPrompt),
		)
	...
	if (shouldPush) {
		...
		if (!remote) {
			failAndExit(
				'No remote found. Add a remote and run: git push && git push <remote> ' +
					tag,
			)
			return
		}
		try {
			...
			failAndExit(
				`Push failed. Run manually: git push && git push ${remote} ${tag}`,
			)
		}
	} else {
		const remote = getDefaultRemote() ?? 'origin'
		console.log(
			`  ${style('→', ui.gray)} Push later with: ${style(`git push && git push ${remote} ${tag}`, ui.cyan)}`,
		)
	}
	console.log()
}
