...
function detectCommitType(staged: StagedContext): string {
	...
	if (
		...
		paths.every(
			(path) =>
				/(^|\/)(readme|docs?|changelog)/.test(path) || path.endsWith('.md'),
		)
	) {
		return 'docs'
	}
	...
	if (
		status.modified > 0 &&
		staged.metrics.insertions <= staged.metrics.deletions
	) {
		return 'fix'
	}
...
/*
 * Summarize staged changes into a compact commit type guess so the prompt can
 * steer the AI toward a message that matches the shape of the diff.
 */
function summarizeChanges(staged: StagedContext): string {
	const status = countByStatus(staged)
	const topFiles = staged.files.slice(0, 3).map((file) => file.path)
	const filesHint =
		staged.files.length > 3
			? `${topFiles.join(', ')} +${staged.files.length - 3} more`
			: topFiles.join(', ')
...
/*
 * Run repo-local validation hooks before generating the final commit message.
 */
...
async function maybePushCurrentBranch(): Promise<void> {
	...
	if (upstream) {
		...
		try {
			execFileSync(
				'git',
				['push', upstream.remote, `${currentBranch}:${upstream.branch}`],
				{
					stdio: 'inherit',
				},
			)
	...
	if (!remote) {
		console.log(
			warn('SKIPPED', 'No git remote configured. Push not available.'),
		)
		return
	}
	...
	try {
		execFileSync('git', ['push', '--set-upstream', remote, currentBranch], {
			stdio: 'inherit',
		})
		console.log(
			ok(
				'OK',
				`Push completed and upstream configured (${remote}/${currentBranch}).`,
			),
		)
	} catch {
...
/*
 * Drive the full commit flow: stage changes, inspect the diff, ask the AI for
 * a message, confirm it, and optionally push the branch or amend the last commit.
 */
export async function generateCommit(
	...
	if (staged.files.length === 0) {
		failAndExit(
			'No staged files.',
			'Run `git add <file>` to stage changes, or use `gemit commit --all` to stage everything.',
		)
	}

	if (staged.metrics.fileCount >= WARN_SPLIT_FILE_COUNT) {
		warnBlock(
			`${staged.metrics.fileCount} staged files detected. Consider splitting into multiple commits for a cleaner history.`,
		)
	}
	...
	if (staged.metrics.patchChars > WARN_PROMPT_DIFF_CHARS) {
		warnBlock(
			`Large staged diff (${staged.metrics.patchChars} chars). The patch sent to the AI will be truncated — commit quality may be reduced.`,
		)
	}
	...
	if (options.dryRun) {
		...
		console.log()
		printList(
			'Would commit these files',
			staged.files.map((f) => `${f.status} ${f.path}`),
		)
	...
	console.log()
	const confirmed = await askConfirmation('Commit using AI message? (Y/n): ', {
		defaultYes: true,
	})
	if (!confirmed) {
	...
	try {
		execFileSync('git', ['commit', '-m', suggestedMessage], {
			stdio: 'inherit',
		})
	} catch {
...
async function amendLastCommit(): Promise<void> {
	...
	try {
		lastCommitHash = execFileSync('git', ['rev-parse', '--short', 'HEAD'], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
		lastCommitMessage = execFileSync('git', ['log', '-1', '--pretty=%B'], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
		...
		failAndExit(
			'Could not read last commit. Make sure there is at least one commit.',
		)
	}

	let commitDiff: string
	try {
		const raw = execFileSync('git', ['show', '--stat', 'HEAD'], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
		commitDiff =
			raw.length > MAX_PROMPT_PATCH_CHARS
				? `${raw.slice(0, MAX_PROMPT_PATCH_CHARS)}\n(truncated)`
				: raw
	} catch {
	...
	const result = await withProgress(
		'AI is generating a better message...',
		() => llm.invoke(prompt),
	)
	const suggestedMessage = extractMessageText(result.content)
	...
	const confirmed = await askConfirmation(
		'Amend commit with this message? (Y/n): ',
		{ defaultYes: true },
	)
	if (!confirmed) {
	...
	try {
		execFileSync('git', ['commit', '--amend', '-m', suggestedMessage], {
			stdio: 'inherit',
		})
