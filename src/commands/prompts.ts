...
export function showPrompts(): void {
	...
	console.log()
	console.log(
		'Run `gemit prompts --init` to export the defaults to your global config.',
	)
	console.log(
		'Run `gemit prompts --edit <name>` to open a specific prompt in your editor.',
	)
	console.log()
}
...
export function initPrompts(): void {
	...
	if (created.length > 0) {
		...
		console.log()
		console.log(
			'All files already exist. Use `gemit prompts --edit <name>` to customize.',
		)
	}
...
export function editPrompt(name: string): void {
	if (!PROMPT_NAMES.includes(name as (typeof PROMPT_NAMES)[number])) {
		console.error(
			`Unknown prompt: "${name}". Valid names: ${PROMPT_NAMES.join(', ')}`,
		)
		process.exitCode = 1
		return
	}
