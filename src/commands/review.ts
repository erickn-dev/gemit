...
export async function reviewStagedChanges(): Promise<void> {
	...
	printKeyValues([
		...
		{
			key: 'Insertions',
			value: style(`+${staged.metrics.insertions}`, ui.green),
		},
	...
	const prompt = REVIEW_PROMPT.replace('{{patch}}', patchContent).replace(
		'{{staged_files}}',
		stagedFiles,
	)

	const result = await withProgress('AI is reviewing your changes...', () =>
		llm.invoke(prompt),
	)
	...
	for (const line of lines) {
		...
		if (trimmed.startsWith('[CRITICAL]')) {
			console.log(
				`  ${style('✗', ui.red, ui.bold)} ${style(trimmed.replace('[CRITICAL]', '').trim(), ui.red)}`,
			)
		} else if (trimmed.startsWith('[WARNING]')) {
			console.log(
				`  ${style('⚠', ui.yellow, ui.bold)} ${style(trimmed.replace('[WARNING]', '').trim(), ui.yellow)}`,
			)
		} else if (trimmed.startsWith('[INFO]')) {
			console.log(
				`  ${style('●', ui.cyan)} ${trimmed.replace('[INFO]', '').trim()}`,
			)
