...
export async function squashCommits(count: number): Promise<void> {
	...
	if (history.length < count) {
		failAndExit(
			`Only ${history.length} commits available. Cannot squash ${count}.`,
		)
	}
	...
	printList(
		...
		commitsToSquash.map(
			(c) => `${style(c.hash.slice(0, 7), ui.yellow)}  ${c.subject}`,
		),
	)
	...
	const commitsText = commitsToSquash
		.map(
			(c) =>
				`- ${c.hash.slice(0, 7)} ${c.subject}${c.body ? `\n  ${c.body}` : ''}`,
		)
		.join('\n')

	const prompt = SQUASH_PROMPT.replace('{{commits}}', commitsText)

	const result = await withProgress('AI is generating squash message...', () =>
		llm.invoke(prompt),
	)
	...
	console.log()
	printKeyValues([
		{ key: 'New message', value: style(suggestedMessage, ui.cyan) },
	])
	console.log()
	...
	try {
		...
		console.log()
		console.log(
			ok('SQUASHED', `${count} commits merged into: ${suggestedMessage}`),
		)
	} catch {
