...
export async function stashWithMessage(): Promise<void> {
	...
	try {
		filesRaw = execFileSync('git', ['diff', '--name-status', 'HEAD'], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
	...
	try {
		const raw = execFileSync('git', ['diff', 'HEAD'], { stdio: 'pipe' })
			.toString()
			.trim()
	...
	const prompt = STASH_PROMPT.replace('{{files}}', filesRaw).replace(
		'{{diff}}',
		diffRaw,
	)

	const result = await withProgress('AI is generating stash message...', () =>
		llm.invoke(prompt),
	)
	...
	console.log()
	printKeyValues([
		{ key: 'Stash message', value: style(suggestedMessage, ui.cyan) },
	])
	console.log()
	const confirmed = await askConfirmation(
		'Stash changes with this message? (Y/n): ',
		{ defaultYes: true },
	)
	if (!confirmed) {
	...
	try {
		execFileSync('git', ['stash', 'push', '-u', '-m', suggestedMessage], {
			stdio: 'inherit',
		})
