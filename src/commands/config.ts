...
/*
 * Interactively collect provider credentials and persist the global .env file
 * used by the CLI runtime.
 */
export async function initConfig(): Promise<void> {
	...
	const providerInput = await askInput(
	...
	const langInput = await askInput(
		'Language for AI responses (en/pt-br) [en]: ',
	)
...
/*
 * Update a single key in the shared .env file while preserving the other
 * configuration entries already stored there.
 */
export function setConfig(keyValue: string): void {
	...
	if (!ALLOWED_KEYS.includes(key as AllowedKey)) {
		failAndExit(
			`Unknown key "${key}". Allowed keys: ${ALLOWED_KEYS.join(', ')}`,
		)
	}
	...
	printKeyValues([
		...
		{
			key: 'Old value',
			value: key.includes('KEY') && oldValue !== '(not set)' ? '***' : oldValue,
		},
...
/*
 * Report whether the current process environment and config file are usable
...
export function doctorConfig(): void {
		...
		provider === 'google'
			...
			: provider === 'openai'
				...
				: provider === 'anthropic'
					? anthropicKey
					: ''
	const expectedKeyName = providerOk
		? getProviderKeyName(provider as Provider)
		: 'API_KEY'
	...
	printKeyValues([
		{
			key: 'LLM_PROVIDER',
			value: providerOk ? ok('ok') : bad('missing/invalid'),
		},
		{ key: 'LLM_MODEL', value: modelOk ? ok('ok') : bad('missing') },
		{
			...
			value: languageOk
				? ok(language)
				: warn(`"${language}" (unknown, using "en")`),
		},
		{
			key: `Expected (${expectedKeyName})`,
			value: expectedKeyOk ? ok('ok') : bad('missing'),
		},
		{ key: 'GOOGLE_API_KEY', value: googleKey ? ok('set') : warn('missing') },
		{ key: 'OPENAI_API_KEY', value: openAiKey ? ok('set') : warn('missing') },
		{
			key: 'ANTHROPIC_API_KEY',
			value: anthropicKey ? ok('set') : warn('missing'),
		},
	])
