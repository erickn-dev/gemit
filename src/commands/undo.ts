...
export async function undoLastCommit(): Promise<void> {
	...
	try {
		lastCommitHash = execFileSync('git', ['rev-parse', '--short', 'HEAD'], {
			...
			.toString()
			.trim()
		lastCommitMessage = execFileSync('git', ['log', '-1', '--pretty=%s'], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
		lastCommitAuthor = execFileSync('git', ['log', '-1', '--pretty=%an'], {
			stdio: 'pipe',
		})
			.toString()
			.trim()
	...
	const confirmed = await askConfirmation(
		'Undo this commit? Files will remain staged. (y/N): ',
	)
	if (!confirmed) {
	...
	try {
		...
		console.log(
			ok(
				'DONE',
				`Commit ${lastCommitHash} was undone. Changes are now staged.`,
			),
		)
	} catch {
