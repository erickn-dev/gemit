import * as aiPrompts from '../aiPrompts.js'
...
function formatCommitsForPrompt(): string {
	...
	if (context.commits.length === 0) {
		failAndExit(
			...
			'Make at least one commit before generating a PR description.',
		)
	}
...
function parsePrSuggestion(raw: string): PrSuggestion {
	...
	if (!titleLine || descriptionIndex < 0) {
		failAndExit(
			'Failed to parse PR suggestion from AI response.',
			'The AI returned an unexpected format. Try running the command again.',
		)
	}
...
/*
 * Build a PR title and description from the current branch's commits and diff,
 * then print the generated text for copy/paste into the hosting provider.
 */
export async function generatePullRequestText(): Promise<void> {
	...
	const template = aiPrompts.loadPrompt('pr')
	const prompt = aiPrompts.interpolate(template, { branch_data: branchData })

	const response = await withProgress(
		'AI is preparing PR title and description...',
		() => llm.invoke(prompt),
	)
