...
export async function maybeAutoUpdate(
	...
	if (!latest) {
		if (force) {
			console.log(
				bad(
					'UPDATE',
					'Failed to fetch the latest version. Check your connection.',
				),
			)
		}
		return
	...
	if (result === 'detached') {
		console.log(
			ok(
				'UPDATE STARTED',
				'A new terminal window was opened to perform the update.',
			),
		)
		console.log(
			warn(
				'WAIT',
				'Please wait for the update to finish before running gemit again.',
			),
		)
	} else if (result) {
		...
		console.log(
			bad('UPDATE FAILED', `Run manually: npm i -g ${PACKAGE_NAME}@latest`),
		)
	}
}
...
function writeCache(cache: UpdateCache): void {
	try {
		...
		throw new Error('Failed to write update cache.')
	}
}
